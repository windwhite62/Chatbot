...
PROMPT = """Tu es l'assistant numerique officiel de la ville de Lambersart (59130, Nord).
...
def fit():
    ...
    if not docs: return
    _vec = TfidfVectorizer(analyzer="word", ngram_range=(1,2), min_df=1, token_pattern=r"[a-zA-Z]{2,}")
...
def context(query, k=3):
    ...
    for i in np.argsort(sims)[::-1][:k]:
        ...
        chunks.append(f"[{d['url']}]\n{d['text'][:800]}")
    return "\n\n---\n\n".join(chunks)

def mistral_call(messages):
    client = Mistral(api_key=MISTRAL_API_KEY)
    if MISTRAL_V1:
        r = client.chat.complete(model=MODEL, messages=messages, temperature=0.2, max_tokens=600)
    else:
...
@app.route("/")
def index():
    return jsonify({"status":"ok","service":"Assistant Lambersart","pages_indexed":len(_index)})

@app.route("/health")
def health():
    return jsonify({"status":"ok","model":MODEL,"indexed":len(_index),"mistral_v1":MISTRAL_V1})
